...
import { collection, query, onSnapshot, serverTimestamp, addDoc, getDoc, doc, getDocs} from 'firebase/firestore';
...
function TimeLineHome() {
  ...
  function mapElements() {
    let eventOutput = events.map(item => <UserEvent key = {item.item.id} input ={item}/>) 
...
function ClickBoxes() {
  ...
  return (
    <Timeline>
      <Events>
        {events.map(event => <UserEvent key = {event.id} event ={event}/>)}
      </Events>
    </Timeline>
